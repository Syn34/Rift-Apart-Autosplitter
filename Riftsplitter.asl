...
//Patch for v3.630.1.0 by Nhekno.

//Below every variable are comments describing the address as seen in Cheat Engine
//Data within () indicate it may or may not contain the code within it between updates
//'?' represents one character present

state("RiftApart")
{
	bool isPortalLoading: 0x6748908, 0x5308;
	//Bytes: 80 54 ?? ?? ?? | Opcode: adc byte ptr [???+???(*?)+-??],???
	//Nhekno: old 2.618.0.0 -> 0x674CAC8, 0x5308
	//0x6741268, 0x5308
	...
	int isLoadingMap: 0x5F90C78;
	//Bytes: 00 00 | Opcode: add [rax],al
	//To find it, 0x5F90C7E has -> Bytes: 80 BF 00000000 00 | Opcode: cmp byte ptr [???+00000000],00
	//Nhekno: old 2.618.0.0 -> 0x5F94EC8
	...
	int cutscene: 0x6A6F8A0, 0x0, 0x208, 0x314;
	//Address: RiftApart.AK::SoundEngine::g_PlayingID+10 | Bytes: 30 8E ???????? | Opcode: xor [???+????????],cl
	//Nhekno: old 2.618.0.0 -> 0x6A73A60, 0x0, 0x208, 0x314
	...
	int levelnr: 0x5161258;
	//Bytes: 02 00 | Opcode: add al,[rax]
	//To find it, 0x516125E has -> Bytes: A1 26C0C4F7A4000000 | Opcode: mov eax,[A4F7C4C026]
	//Nhekno: old 2.618.0.0 -> 0x5165298
